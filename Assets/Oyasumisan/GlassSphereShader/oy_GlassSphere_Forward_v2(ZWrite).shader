...
////// Specular:
                ...
                float2 node_7424 = mul( UNITY_MATRIX_V, float4((-1*reflect(viewReflectDirection,i.normalDir)),0) ).xyz.rgb.rg;
                float node_5222 = pow(1.0-max(0,dot(normalDirection, viewDirection)),_Distortion);
                float node_8313 = (1.0 - _Use_Distortion);
                float2 node_8665 = (((i.uv0+(node_7424*node_5222))*_Use_Distortion)+(i.uv0*node_8313));
                float4 _Texture_var = tex2D(_Texture,TRANSFORM_TEX(node_8665, _Texture));
                float node_3049 = (_TextureOpacity*-1.0+1.0);
                float4 node_7157 = tex2D( _GrabTexture, ((node_8313*sceneUVs.rg)+((sceneUVs.rg+(node_7424*node_5222))*_Use_Distortion)));
                float3 diffuseColor = ((_Albedo_Base_Color.rgb+(((_Texture_var.rgb*node_3049*_Texture_var.a)+(node_7157.rgb*(_Texture_var.a*-1.0+1.0)*node_3049)+(node_7157.rgb*_TextureOpacity))*_Incident_Color.rgb))*_Exposure); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float node_3049 = (_TextureOpacity*-1.0+1.0);
                float4 node_7157 = tex2D( _GrabTexture, ((node_8313*sceneUVs.rg)+((sceneUVs.rg+(node_7424*node_5222))*_Use_Distortion)));
                float3 diffColor = ((_Albedo_Base_Color.rgb+(((_Texture_var.rgb*node_3049*_Texture_var.a)+(node_7157.rgb*(_Texture_var.a*-1.0+1.0)*node_3049)+(node_7157.rgb*_TextureOpacity))*_Incident_Color.rgb))*_Exposure);
